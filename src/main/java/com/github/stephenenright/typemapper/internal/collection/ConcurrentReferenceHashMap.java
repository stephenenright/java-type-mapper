package com.github.stephenenright.typemapper.internal.collection;
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	public boolean containsKey( Object key) {
		Entry<K, V> entry = getEntryIfAvailable(key);
		return (entry != null && ObjectUtils.areEqual(entry.getKey(), key));
	}
	...
	public boolean remove(Object key, final Object value) {
		Boolean result = doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
			...
			protected Boolean execute( Reference<K, V> reference,  Entry<K, V> entry) {
				if (entry != null && ObjectUtils.areEqual(entry.getValue(), value)) {
					if (reference != null) {
						reference.release();
					}
					return true;
	...
	public boolean replace(K key, final V oldValue, final V newValue) {
		Boolean result = doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
			@Override
			protected Boolean execute( Reference<K, V> reference,  Entry<K, V> entry) {
				if (entry != null && ObjectUtils.areEqual(entry.getValue(), oldValue)) {
			...
			while (currRef != null) {
				if (currRef.getHash() == hash) {
					...
					if (entry != null) {
						...
						if (ObjectUtils.areEqual(entryKey, key)) {
							return currRef;
						}
					}
				}
	...
	protected static final class Entry<K, V> implements Map.Entry<K, V> {
		...
		public final boolean equals(Object other) {
			...
			return (ObjectUtils.areEqual(getKey(), otherEntry.getKey()) &&
					ObjectUtils.areEqual(getValue(), otherEntry.getValue()));
		}
	...
	private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public boolean contains( Object o) {
			if (o != null && o instanceof Map.Entry<?, ?>) {
				...
				if (other != null) {
					return ObjectUtils.areEqual(entry.getValue(), other.getValue());
				}
			}
			return false;
		}
